...
function App() {
  ...
  const UpdateModalWithStarData = (starId) => {
    LookUpID(starId).then((result) => {
      ...
      setMessage(
        ` ** [${dec_RAD}, ${ra_RAD}], // Constellation: ${result[0].bayerFlamsteed} `
      );
  ...
  useEffect(() => {
    if (!lastData && !(promising instanceof Promise)) {
      // when program is first loaded and lastData is empty/undefined
      ...
      fov !== lastDataCoords[2]
    ) {
      lastDataCoords = [currentDecRa.DecCurrent, currentDecRa.RaCurrent, fov];
      LookUp(fov, currentDecRa.DecCurrent, currentDecRa.RaCurrent).then(
        (result) => {
          lastData = { ...result };
          setStarData(lastData);
          dataAge = Date.now();
